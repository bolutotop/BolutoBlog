  ...
  return (
    <div id="showcase-root" className="showcase-theme min-h-screen font-sans selection:bg-black selection:text-white transition-colors duration-700 overflow-x-hidden relative">
      ...
      {isFirstVisit && bootStage < 2 && (
        <div ref={preloaderRef} className="fixed inset-0 z-[99999] bg-[#050505] text-white flex flex-col justify-between p-8 md:p-12 font-mono">
          ...
          <div className="flex flex-col items-center justify-center">
            <div ref={counterRef} className="text-[20vw] md:text-[15rem] font-black leading-[0.8] tracking-tighter">000%</div>
      ...
      <style jsx global>{`
        ...
        /* 🚀 终极修复：侧边栏隐藏与主容器扩展的平滑动画逻辑 */
        #sidebar-left, #sidebar-right {
          /* 使用独立的 translate 和 opacity 进行过渡，彻底避开 transform 的冲突 */
      ...
      {bootStage >= 1 && (
        <>
          <header className="fixed top-0 left-0 w-full px-6 py-5 md:px-8 z-50 flex justify-between items-center sc-border border-b backdrop-blur-md bg-[var(--sc-bg)]/80 transition-colors duration-700 pointer-events-none animate-header-layout">
            <div className="font-black text-xl tracking-tighter uppercase pointer-events-auto">ZHIHUI.</div>
            <nav className="hidden md:flex gap-8 text-xs font-bold uppercase tracking-widest pointer-events-auto">
          ...
          <aside id="sidebar-left" className="fixed left-0 top-0 h-screen w-64 sc-border border-r z-40 flex-col justify-between pt-28 pb-10 px-8 hidden xl:flex pointer-events-none animate-sidebar-left">
            <div className="pointer-events-auto">
              <div className="w-16 h-16 rounded-full overflow-hidden mb-5 sc-border border-2">
              ...
              <p className="text-[10px] font-bold mt-2 uppercase tracking-widest opacity-50">Creative Developer</p>
            </div>

            <div className="pointer-events-auto">
              <div className="text-[10px] font-black uppercase tracking-widest mb-4 sc-border border-b pb-2 opacity-50">Core Stack</div>
          ...
          <aside id="sidebar-right" className="fixed right-0 top-0 h-screen w-72 sc-border border-l z-40 flex-col pt-28 pb-10 px-6 hidden xl:flex pointer-events-none animate-sidebar-right">
            <div className="pointer-events-auto flex items-end justify-between sc-border border-b pb-4">
              <div>
                <div className="text-5xl font-black tabular-nums tracking-tighter leading-none">{dateInfo.day}</div>
                <div className="text-[10px] font-bold uppercase tracking-widest mt-1 opacity-50">{dateInfo.month}</div>
              </div>
              <div className="flex items-center gap-1.5 mb-1">
                <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span>
                <span className="text-[8px] font-black uppercase tracking-widest opacity-50">SG</span>
              </div>
            </div>
            ...
            <div className="pointer-events-auto mt-8 relative z-20">
              ...
              <div className="grid grid-cols-7 gap-1 text-center font-mono text-[9px] font-bold uppercase mb-4 opacity-40">
                {weekDays.map((d, i) => <div key={i}>{d}</div>)}
              </div>
              
              <div className="grid grid-cols-7 gap-y-2 gap-x-1 font-mono text-xs">
              ...
              {activeModal && (
                <>
                  ...
                  <div className={`absolute top-0 right-[110%] w-80 bg-[var(--sc-bg)] sc-border border p-6 shadow-2xl z-50 pointer-events-auto ${isClosing ? 'animate-panel-exit' : 'animate-panel-enter'}`}>
                    ...
                    <div className="flex justify-between items-start mb-5 sc-border border-b pb-3">
                       ...
                       <button onClick={handleCloseModal} className="text-[10px] font-black uppercase hover:opacity-50 transition-opacity">
                         [ Close ]
                       </button>
                    </div>
            ...
            <div className="pointer-events-auto mt-auto pt-6 sc-border border-t">
               <div className="text-[10px] font-black uppercase tracking-widest opacity-50 mb-1">System Status</div>
               <div className="text-xs font-bold uppercase tracking-tight">All systems operational.</div>
            </div>
          </aside>
