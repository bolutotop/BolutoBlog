"use client";

import React from 'react';
import Link from 'next/link';
// 🚀 引入我们刚刚封装的搜索组件
import SearchWidget from './SearchWidget';
import MobileMenu from './MobileMenu';

interface HeaderProps {
  isMobileMenuOpen: boolean;
  setIsMobileMenuOpen: React.Dispatch<React.SetStateAction<boolean>>;
  dateInfo: { day: string; month: string };
  categories: string[];
  showAllCats: boolean;
  setShowAllCats: React.Dispatch<React.SetStateAction<boolean>>;
}

export default function Header({ 
  isMobileMenuOpen, 
  setIsMobileMenuOpen,
  dateInfo,
  categories,
  showAllCats,
  setShowAllCats
}: HeaderProps) {
  return (
    <>
      <header className="fixed top-0 left-0 w-full px-5 py-2 md:px-8 z-50 flex justify-between items-center sc-border border-b backdrop-blur-md bg-[var(--sc-bg)]/80 transition-colors duration-700 pointer-events-none animate-header-layout">
        
        {/* 1. 左侧 Logo */}
        <Link href="/" className="font-black text-lg md:text-[clamp(1.25rem,1.5vw,1.75rem)] tracking-tighter uppercase pointer-events-auto relative z-50 shrink-0">
          ZHIHUI.
        </Link>
        
        <div className="flex items-center gap-4 md:gap-8 pointer-events-auto relative z-50">
          
          {/* ================================================= */}
          {/* 🚀 中间：独立出来的搜索组件 */}
          {/* ================================================= */}
          <SearchWidget 
            inputRadius="rounded-full" 
            dropdownRadius="rounded-2xl" 
            animationSpeed="duration-500" 
          />

          {/* ================================================= */}
          {/* 🚀 2. 右侧导航：删除了 hidden md:flex，去掉了“主页” */}
          {/* ================================================= */}
          <nav className="flex gap-4 md:gap-8 text-[10px] md:text-xs font-bold uppercase tracking-widest shrink-0">
            <Link href="/" className="hover:line-through">主页</Link>
            <Link href="/blog" className="hover:line-through">日志</Link>
            <Link href="/archive" className="hover:line-through">归档</Link>
          </nav>

          {/* 3. 移动端汉堡菜单 (保留用于打开分类抽屉) */}
          <button 
            className="xl:hidden w-6 h-6 flex flex-col justify-center items-center group ml-1 shrink-0"
            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
            aria-label="Toggle Menu"
          >
            <span className={`block absolute w-5 h-[1.5px] bg-current transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'rotate-45' : '-translate-y-1.5'}`}></span>
            <span className={`block absolute w-5 h-[1.5px] bg-current transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'opacity-0' : 'opacity-100'}`}></span>
            <span className={`block absolute w-5 h-[1.5px] bg-current transition-all duration-300 ease-in-out ${isMobileMenuOpen ? '-rotate-45' : 'translate-y-1.5'}`}></span>
          </button>
        </div>
      </header>

      <MobileMenu 
        isMobileMenuOpen={isMobileMenuOpen}
        setIsMobileMenuOpen={setIsMobileMenuOpen}
        dateInfo={dateInfo}
        categories={categories}
        showAllCats={showAllCats}
        setShowAllCats={setShowAllCats}
      />
    </>
  );
}